...
class Image {
    ...
    /**
     * 为图片添加水印
     * @param $dst string 原图路径
     * @param $src string 需要裁剪的宽
     * @param $savename string 保存的路径
     * @param $savename string 保存路径
     * @param $root_water_path_path string 水印图片文件路径
     * http://www.php100.com/html/php/lei/2013/0905/5342.html
     */
    static public function water($dst, $src, $savename,$root_water_path) {
    	if (!is_dir($root_water_path))mkdir($root_water_path,0777);
    	//原始图像
    	//得到原始图片信息
    	...
    	switch ($dst_info[2]){
    		case 1:
    			imagegif($dst_im,$savename);break;
    		case 2:
    			imagejpeg($dst_im,$savename);break;
    		case 3:
    			imagepng($dst_im,$savename);break;
    		case 6:
    			imagewbmp($dst_im,$savename);break;
    		default:
    			die("不支持的文件类型2");exit;
    	}
    	imagedestroy($dst_im);
     ...
     * @param $savename string 保存路径
     * @param $root_cropper_path string 剪裁图片文件路径
     * http://www.thinkphp.cn/code/605.html
     */
    function imagecropper($source_path, $target_width, $target_height,$savename,$root_cropper_path)
    {
    	if (!is_dir($root_cropper_path))mkdir($root_cropper_path,0777);
    	...
    	//保存图片到本地(两者选一)
    	imagepng($target_image,$savename);
...
// 		imagedestroy($cropped_image);
    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        ...
        if ($info !== false) {
            ...
            $imageFun($thumbImg, $thumbname,C('PICTURE_S_ZIP_RATE'));
